 ...
 *  \file qcustomplot.h
 *
 *  \details Файл библиотеки для построения графиков <a href="https://www.qcustomplot.com/documentation/classQCustomPlot.html">QCustomPlot</a>
 */
...
public:
  ...
  virtual void setExternalPainter(QCPPainter */*painter*/) {};
  virtual void freePainer(QCPPainter* /*painter*/) = 0;
...
public:
  ...
  void freePainer(QCPPainter* painter) Q_DECL_OVERRIDE;
...
class QCP_LIB_DECL QCPPaintBufferDirect : public QCPAbstractPaintBuffer
{
    QCPPainter* mPainter;

public:
  explicit QCPPaintBufferDirect(const QSize &size, double devicePixelRatio);
  virtual ~QCPPaintBufferDirect();
  ...
  void setExternalPainter(QCPPainter *painter) Q_DECL_OVERRIDE;
  void freePainer(QCPPainter* painter) Q_DECL_OVERRIDE;
...
public:
  ...
  */
  ...
  Qt::MouseButtons dragButtons() { return mDragButtons; }
  ...
  void setDirectPainting(bool enable);

  void setDragButtons(Qt::MouseButtons buttons);
...
protected:
  ...
  void setupPaintBuffers(QCPPainter *painter = nullptr);
...
public:
  ...
  double key, value;
  uint index; // Нужна для сохранения изначального индекса (номера по порядку) этих данных только для графика "Взаимозависимостей"
              // для определения цвета точки
};
...
public:
  ...
  */
  ...
  double boolGraphPosition = 0;
  bool transparentZeroLine = false;
...
protected:
  ...
  virtual void drawBoolPlot(QCPPainter *painter, const QVector<QPointF> &lines);
  ...
  QVector<QPointF> dataToBoolLines(const QVector<QCPGraphData> &data) const;
