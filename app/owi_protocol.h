...
//#ifndef OWI_PROTOCOL_H
//#define OWI_PROTOCOL_H
...
//#define OWI_setReg(_REG, _VAL, _MEM) OWI_CMD_Data16((_REG + _MEM + 0x70), _VAL)


//#define CNT_REGS_CLC_CRC 0x1D   // Количество регистров, для которых нужно считать CRC
//#define MAX_REGS 32             // Всего регистров
//#define OWI_H { fp=fopen(GPIO_PIN_PATH_OWI"/value","w"); fprintf(fp,"1"); fclose(fp); }
//#define OWI_L { fp=fopen(GPIO_PIN_PATH_OWI"/value","w"); fprintf(fp,"0"); fclose(fp); }
...
//#endif // OWI_PROTOCOL_H

////bool initOWI(char *DEV_NAME)
////{
...
////    if (ret == -1)
////    {
////        qDebug("Unable to get chip info from ioctl");
////        close(fd);
////        return false;
////    }
////    qDebug() << "Chip name: %%s" << info.name;
////    qDebug() << "Chip label: %%s" << info.label;
////    qDebug() << "Number of lines: %%d" << info.lines;
