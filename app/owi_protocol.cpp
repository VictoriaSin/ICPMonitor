...
//struct spi_ioc_transfer mWriteReadMessage[] = { mMessageTX, mMessageRX };
...
//bool spi_transferSPI(uint8_t const *TX, u8 txLen, uint8_t *RX, u8 rxLen)
//{
//  // Если устройство не готово к чтению и записи
//  mMessageTX.tx_buf = (unsigned long)TX;
//  mMessageTX.len = txLen;
//  mMessageRX.rx_buf = (unsigned long)RX;
//  mMessageRX.len = rxLen;
//  CS_L();
//  // Передаём сообщение
//  if (ioctl(mFd, SPI_IOC_MESSAGE(1), &Message) < 1)
//  {
//    CS_H();
//    return false;
//  }
...
//{
//  CS_L();
//  if (ioctl(mFd, SPI_IOC_MESSAGE(2), &mWriteReadMessage) < 2) { CS_H(); qDebug() << "error"; return false; }
//  CS_H();
//  return true;
//}
...
#define PIN_OWI   GPIO_PIN_PATH_OWI
...
void OWI_InitPins()
{
  ...
  qDebug() << "pin init";
  //  fp=fopen(GPIO_PIN_PATH_OWI"/direction","w");
  //  fprintf(fp,"out");
  //  fclose(fp);
  //
  //  fp=fopen(GPIO_PIN_PATH_OWI"/active_high","w");
  //  fprintf(fp,"1");
  //  fclose(fp);
}
